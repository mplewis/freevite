...
const ShowError = ({ error }: { error: Error }) => (
  <>
    ...
    <Typ x='p'>
      Sorry, we&apos;re having trouble fulfilling your unsubscribe request. Please email{' '}
      <a href='mailto:support@freevite.com'>support@freevite.com</a> and we will make sure you are
      permanently unsubscribed from our emails.
    </Typ>
  </>
)

const UnsubscribeForm = ({ email, token }: { email: string; token: string }) => {
  const [save, { loading, error }] = useMutation<UnsubscribeMutation, UnsubscribeMutationVariables>(
  ...
  return (
    <>
      ...
      <Typ x='p'>
        You are about to unsubscribe from <strong>all</strong> Freevite emails. After this, Freevite
        will not email you for <strong>any reason.</strong> This will prevent you from creating an
        event or RSVPing to an event, as we need to send you a confirmation link via email.
      </Typ>
      <Typ x='subhead'>Unsubscribe</Typ>
      <Typ x='p'>To confirm you want to unsubscribe, click the button below:</Typ>
      <button
...
const ResubscribeForm = ({ email, token }: { email: string; token: string }) => {
  const [save, { loading, error }] = useMutation<ResubscribeMutation, ResubscribeMutationVariables>(
  ...
  return (
    <>
      ...
      <Typ x='p'>
        Freevite will not email you for <strong>any reason.</strong> This will prevent you from
        creating an event or RSVPing to an event, as we need to send you a confirmation link via
        email.
      </Typ>
      <Typ x='subhead'>Resubscribe</Typ>
      <Typ x='p'>If you want to start receiving Freevite emails again, click the button below:</Typ>
      <button
...
export const Failure = ({ error }: CellFailureProps<IgnoredEmailQueryVariables>) => {
  console.error({ error })
  return (
    <DeadEnd
